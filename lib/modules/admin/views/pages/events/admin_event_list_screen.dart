...
import 'package:badminton_ai/modules/admin/views/pages/events/admin_create_event_screen.dart';
import 'package:badminton_ai/modules/admin/viewmodels/admin_events_viewmodel.dart';
...
class _AdminEventListViewState extends State<_AdminEventListView> {
  void _confirmDelete(BuildContext context, EventModel event) {
    DialogUtils.showConfirmDialog(
      context,
      title: 'screens.confirmDeletion'.tr(),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: CustomGradientAppBar(
        title: Text('screens.eventManagement1'.tr()),
      ),
      body: Column(
        children: [
          // Filter Section
          Container(
            ...
            child: Row(
              ...
              children: [TimeFilterWidget(viewModel: vm)],
            ),
          ),

          Expanded(
            child: StreamBuilder<List<EventModel>>(
              stream: context.read<SupabaseRepository>().getEventsStream(
                ownerId: ownerId,
              ),
              builder: (context, snapshot) {
                ...
                if (snapshot.hasError) {
                  return Center(child: Text('screens.errorLoadingData'.tr()));
                }
      ...
      floatingActionButton: FloatingActionButton.extended(
        onPressed: () async {
          if (isCourtOwner) {
            ...
            final courts = await repo
                .getCourtLocationsStream(ownerId: ownerId)
                .first;
            if (!context.mounted) return;
            if (courts.isEmpty) {
              AppToast.show(
                context,
                'screens.youNeedToAddAFieldBefore'.tr(),
